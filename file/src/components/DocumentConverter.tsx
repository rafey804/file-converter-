...
export default function DocumentConverter({
  ...
  const generateSEOConfig = useCallback((type: string): ConversionConfig => {
    ...
    return {
      ...
      keywords: `${type.replace('_', ' ')}, ${fromFormat.toLowerCase()}, ${toFormat.toLowerCase()}, document converter, online converter, free converter`,
      urlSlug: type.replace('_', '-'),
  ...
  const handleConversionTypeChange = useCallback((newConversionType: string) => {
    const newUrlSlug = newConversionType.replace('_', '-');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-amber-50">
      <Head>
        ...
        <link rel="canonical" href={`https://flipfilex.com/${seoConfig.urlSlug}`} />
      </Head>
